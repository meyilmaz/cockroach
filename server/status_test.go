...
func startServerAndGetStatus(t *testing.T, keyPrefix string) (*TestServer, []byte) {
	...
	// Make sure the range and spun up with an arbitrary read command. We do not
	// expect a response.
	...
	if err := ts.node.publishStoreStatuses(); err != nil {
		t.Fatalf("error publishing store statuses: %v", err)
	}
	canary := &status.CanaryEvent{}
	canary.Add(1)
	ts.EventFeed().Publish(canary)
	canary.Wait()
	if err := ts.writeSummaries(); err != nil {
		t.Fatalf("error writing summaries: %v", err)
	}
